...
<body>
	...
	<form action="/Board_psy/boardList.do" id="form" name="form"
		method="post">
		<input type="hidden" name="totalBoardCount"
			value="${boardPage.totalBoardCount}"> <input type="hidden"
			name="page"> <input type="hidden" name="prePage"
			value="${boardPage.prePage}"> <input type="hidden"
			name="nextPage" value="${boardPage.nextPage}">
		...
		<table>
			...
			<tr>
				<td class="search_layout">
					<table class="search_table">
						<tr>
							<td class="search_td"><select name="searchKey"
								class="combobox">
									<option value="ALL"
										<c:if test="${searchKey=='ALL'}">selected</c:if>>전체</option>
									<option value="CONTENT"
										<c:if test="${searchKey=='CONTENT'}">selected</c:if>>내용</option>
									<option value="TITLE"
										<c:if test="${searchKey=='TITLE'}">selected</c:if>>제목</option>
									<option value="NAME"
										<c:if test="${searchKey=='NAME'}">selected</c:if>>작성자</option>
							</select> <input type="text" name="searchValue" value="${searchValue}"
								class="txtbox" size="25" /> <!-- button onclick="fn_pageMove(1)">검색</button -->
			...
			<tr>
				<td class="totalBoard_layout">총게시글 수 :
					${boardPage.totalBoardCount}</td>
			...
			<tr>
				<td class="list_layout">
					...
					<table border="1" width
						: 1024px; style="border-collapse: collapse;" class="boardList">
						...
						<c:if test="${!empty boardPage.boardList }">
							...
							<c:forEach items="${boardPage.boardList}" var="board">
								<tr>
									...
									<td id="boardList_title" width="300px"><c:if
											test="${board.indent == 0 && board.flag==1}">
											<a
												href="/Board_psy/read.do?boardNo=${board.boardNo}&isHitCount=true&page=${boardPage.pageNo}&searchKey=${searchKey}&searchValue=${searchValue}">${board.title}</a>
										</c:if> <c:if test="${board.indent == 0 && board.flag==0}">
											<span
												style="font-style: italic; color: gray; font-size: x-small;">
												<a
												href="/Board_psy/read.do?boardNo=${board.boardNo}&isHitCount=true&page=${boardPage.pageNo}&searchKey=${searchKey}&searchValue=${searchValue}">삭제된
													글입니다</a>

											</span>
										</c:if> <c:if test="${board.indent > 0 && board.flag ==1}">
											...
											<span
												style="font-style: italic; color: gray; font-size: x-small;">
												<a
												href="/Board_psy/read.do?boardNo=${board.boardNo}&isHitCount=true&page=${boardPage.pageNo}&searchKey=${searchKey}&searchValue=${searchValue}">
													ㄴ&nbsp;삭제된 글입니다</a>
											</span>

										</c:if></td>
									<td align="center" width="100px"><c:choose>
											<c:when test="${!empty board.files }">
												<c:forEach items="${board.files}" var="file">
													<c:choose>
														<c:when
															test="${!empty sessionScope.email  && board.flag == 1}">
															<a
																href="/Board_psy/download.do?savedPath=${file.savedPath}">
																<img alt="${file.originalName}"
																src="/Board_psy/img/file.jpg" width="20px" height="20px">
															</a>
														</c:when>
														<c:otherwise>
															<img alt="${file.originalName}"
							...
							<tr>
								<td colspan="6" align="center"><a
									href="javascript:fn_pageMove(1)"> &nbsp;<<&nbsp; </a> <a
									href="javascript:fn_pageMove(${boardPage.prePage})"> &nbsp;
										< &nbsp;</a> <c:forEach var="num" begin="${boardPage.startPage}"
										end="${boardPage.endPage}">
										<c:choose>
											<c:when test="${num eq boardPage.pageNo}">
												<a href="javascript:fn_pageMove(${num})" class="choice"
													style="font-weight: bold;">[${num}]</a>
									...
									</c:forEach> <a href="javascript:fn_pageMove(${boardPage.nextPage})">&nbsp;>&nbsp;</a>
			...
			<tr>
				<td class="footer_layout"><input type="hidden"
					value="${sessionScope.email }" name="email"> <%-- 	<c:if test="${!empty sessionScope.email}"> --%>
					<input type="button" id="writeForm" name="writeForm" value="Write"
					style="color: navy;"> <%-- 	</c:if> --%> <c:if
						test="${empty sessionScope.email }">
		로그인 후 게시글을 작성하세요!
	</c:if></td>
			</tr>

		</table>
		<input type="hidden" name="isHitCount" /> <input type="hidden"
			name="boardNo" />
	</form>
</body>
<script type="text/javascript">
